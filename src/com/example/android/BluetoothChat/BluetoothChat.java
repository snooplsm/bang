...
//				Intent intent = new Intent(BluetoothChat.this,CaptureActivity.class);
...
//		        startActivityForResult(intent, 0);
	...
	private void deRegisterListeners() {
		...
		if (pistol != null) {
			pistolContainer.setOnTouchListener(null);
		}
	}
	...
	private void beginGame(BeginGame game) {
		...
		this.beginGame = game;
		Log.i("bang", "setting can shoot to false, current value is "
				+ canShoot);
		canShoot = false;
		clearViews();
		timer = new Timer();
		TimerTask task = new TimerTask() {
			@Override
			public void run() {
				Log.i("bang", "setting can shoot to true, current value is "
						+ canShoot);
				canShoot = true;
				mVibrator.vibrate(120);
			}
		};
		long time = (long) (beginGame.getSecondsUntilDuel() * 1000f - (isClient ? 90
				: 0));
	...
	private View getWinLostGameOverlayView(boolean won) {
		...
		if (won) {
			...
			winLostGameOverlay.setTextColor(Color.parseColor("#D4B81E"));
		} else {
	...
	private void registerListeners() {
		Log.i("bang", "registering listeners");
		sensorManager.registerListener(this, gyroscope,
				SensorManager.SENSOR_DELAY_GAME);
		pistolContainer.setOnTouchListener(this);
	}
}